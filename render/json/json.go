...
func (res *Render) Response(code int, response interface{}) error {
	res.value.Header().Set("Content-Type", "application/json")
	res.value.WriteHeader(code)
	return json.NewEncoder(res.value).Encode(response)
}

// Send sends a JSON-encoded response in the body of a request with the 200 status code.
func (res *Render) Send(response interface{}) error {
	return res.Response(http.StatusOK, response)
}

// Created sends a JSON-encoded response in the body of a request with the 201 status code.
func (res *Render) Created(response interface{}) error {
	return res.Response(http.StatusCreated, response)
}
...
func (res *Render) BadRequest(err error) error {
	message := HTTPError{
		Status:  http.StatusBadRequest,
		Errors:  http.StatusText(http.StatusBadRequest),
		Message: err.Error(),
	}
	return res.Response(http.StatusBadRequest, message)
}
...
func (res *Render) NotFound(err error) error {
	message := HTTPError{
		Status:  http.StatusNotFound,
		Errors:  http.StatusText(http.StatusNotFound),
		Message: err.Error(),
	}
	return res.Response(http.StatusNotFound, message)
}
...
func (res *Render) MethodNotAllowed(err error) error {
	message := HTTPError{
		Status:  http.StatusMethodNotAllowed,
		Errors:  http.StatusText(http.StatusMethodNotAllowed),
		Message: err.Error(),
	}
	return res.Response(http.StatusMethodNotAllowed, message)
}
...
func (res *Render) InternalServerError(err error) error {
	message := HTTPError{
		Status:  http.StatusInternalServerError,
		Errors:  http.StatusText(http.StatusInternalServerError),
		Message: err.Error(),
	}
	return res.Response(http.StatusInternalServerError, message)
}
